...
const points = [
  'It shall be located at an adequate distance (1000 m) in rear of the stop signal it pre-warns.',
  'In colour light signaling, it is equipped with a ‗P‘ marker.',
  'It is capable of showing Caution, Attention and Proceed aspects.',
]

const combinedIntro =
  'Under approved special instructions, a colour light Distant signal may be combined with the last Stop signal of a station in rear or with an Intermediate Block signal or with a Stop signal protecting a level crossing. When a colour Distant signal is combined:'

const combinedItems = [
  { roman: '(i)', text: 'with the last Stop signal of a station in rear, or' },
  { roman: '(ii)', text: 'with an Intermediate Block signal, or' },
  { roman: '(iii)', text: 'with a Stop signal protecting a level crossing,' },
]

const combinedNote =
  'The ‗P‘ marker is dispensed with and normal aspect is stop and shows red light in ‗ON‘.'

const trailingPoints = [
  'Where necessary, two Distant signals may be provided in the same direction where the sectional speed is more than 110 kmph. In this case, Distant is capable of showing Attention and Proceed aspects only.',
  'Distant in double distant area is placed at 2000 m from FSS identified by ‗P‘ mark board.',
  'Whenever Double Distant signal is provided, Signal Warning Board is dispensed.',
]

const DistantSignalPage = () => {
  return (
        ...
        <article className="overflow-hidden rounded-3xl border border-slate-500/30 bg-slate-900/50 shadow-[0_24px_80px_-12px_rgba(0,0,0,0.55)] ring-1 ring-white/6 backdrop-blur-xl animate-[fade-up_0.7s_ease-out]">
          <header className="relative border-b border-amber-500/15 bg-linear-to-br from-slate-900/90 via-slate-900/70 to-amber-950/30 px-5 py-8 sm:px-10 sm:py-10">
            ...
            <div className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-2xl bg-linear-to-br from-amber-400/25 to-orange-600/30 ring-1 ring-amber-400/40 shadow-lg shadow-amber-950/40 animate-[soft-pulse_3.5s_ease-in-out_infinite]">
              <SignalLow className="h-7 w-7 text-amber-200" strokeWidth={1.75} />
            </div>
          ...
          <div className="space-y-4 px-5 py-8 sm:px-10 sm:py-10">
            ...
            <section className="space-y-3 rounded-2xl border border-slate-600/40 bg-slate-900/40 p-4 ring-1 ring-white/4 sm:p-5">
              <p className="flex gap-3.5 text-[15px] leading-[1.75] text-slate-300 sm:text-base">
                <span className="mt-0.5 shrink-0 text-amber-300" aria-hidden>
                  →
                </span>
                <span>{combinedIntro}</span>
              </p>
            ...
            {trailingPoints.map((text, index) => (
              <div
                key={text}
                className="group flex gap-3.5 rounded-2xl border border-slate-600/40 bg-slate-900/40 p-4 shadow-sm ring-1 ring-white/4 transition-all duration-300 hover:border-amber-500/30 hover:bg-slate-900/60 hover:shadow-md hover:shadow-amber-950/20 sm:gap-5 sm:p-5"
                style={{ animation: `fade-up 0.55s ease-out ${0.08 * (index + 5)}s both` }}
              >
                <span className="mt-0.5 shrink-0 text-amber-300" aria-hidden>
                  →
                </span>
...
export default DistantSignalPage
