'use client'

import React from 'react'
import Link from 'next/link'
import { Bomb } from 'lucide-react'

const overviewPoints = [
  {
    number: '1',
    title: 'Maintenance:',
    text: 'The station detonator register is maintained at every station.',
  },
  {
    number: '2',
    title: 'Contents:',
    text: 'It includes the name of the fog signalman, stock and utilization of detonators.',
  },
  {
    number: '3',
    title: 'Knowledge of Rules:',
    text: 'The Station Master (SM) must obtain the signature or thumb impression of all fog signalmen, confirming their knowledge of rules for fog signaling of trains.',
  },
  {
    number: '4',
    title: 'Usage Recording:',
    text: 'The usage of detonators is mentioned in the register. Both the SM and the fog signalman must sign the entry.',
  },
  {
    number: '5',
    title: 'Return of Detonators:',
    text: 'After duty hours, the fog signalman must bring both used and unused detonators and hand them over to the SM, who will enter the details in the register. Both must sign the entry.',
  },
]

const registerParts = [
  'Part I: Particulars of fog signalmen posted at the station.',
  'Part II: Particulars of receipt and stock of detonating signals at the station, to be filled whenever detonators are received.',
  'Part III: Periods of fog, fog signalmen on duty, and details of detonators used.',
  'Part IV: Particulars of issue and testing of detonators at depot station, loco shed, etc.',
]

const closingPoints = [
  {
    number: '7',
    title: 'Frequency:',
    text: 'The register is maintained for every quarter.',
  },
  {
    number: '8',
    title: 'Signatures:',
    text: 'In Part I, the signature of the fog signalman is taken as having knowledge of using the detonator, along with name and designation.',
  },
  {
    number: '9',
    title: 'Inspection:',
    text: 'The Traffic Inspector (TI) must check the register and stock on hand each time they visit the station.',
  },
]

function ArrowList({ items }: { items: string[] }) {
  return (
    <ul className="mt-3 space-y-2 border-l-2 border-amber-500/25 pl-4">
      {items.map((text) => (
        <li key={text} className="flex gap-2.5">
          <span className="mt-0.5 shrink-0 text-sm font-bold text-amber-200"></span>
          <p className="text-[15px] leading-[1.75] text-slate-300 sm:text-base">{text}</p>
        </li>
      ))}
    </ul>
  )
}

const StationDetonatorRegisterPage = () => {
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#0a0c10]">
      <div className="pointer-events-none absolute inset-0 bg-linear-to-b from-slate-950 via-[#0d1117] to-slate-950" />
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_100%_80%_at_50%_-30%,rgba(251,191,36,0.14),transparent_55%)]" />
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_60%_40%_at_100%_50%,rgba(59,130,246,0.06),transparent)]" />
      <div className="pointer-events-none absolute bottom-0 left-0 right-0 h-32 bg-linear-to-t from-amber-950/20 to-transparent" />

      <div className="relative mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:py-16">
        <article className="overflow-hidden rounded-3xl border border-slate-500/30 bg-slate-900/50 shadow-[0_24px_80px_-12px_rgba(0,0,0,0.55)] ring-1 ring-white/6 backdrop-blur-xl animate-[fade-up_0.7s_ease-out]">
          <header className="relative border-b border-amber-500/15 bg-linear-to-br from-slate-900/90 via-slate-900/70 to-amber-950/30 px-5 py-8 sm:px-10 sm:py-10">
            <div className="absolute inset-x-8 top-0 h-px bg-linear-to-r from-transparent via-amber-400/40 to-transparent" />

            <div className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-2xl bg-linear-to-br from-amber-400/25 to-orange-600/30 ring-1 ring-amber-400/40 shadow-lg shadow-amber-950/40 animate-[soft-pulse_3.5s_ease-in-out_infinite]">
              <Bomb className="h-7 w-7 text-amber-200" strokeWidth={1.75} />
            </div>

            <h1 className="text-center text-xl font-bold tracking-tight text-transparent bg-clip-text bg-linear-to-r from-amber-100 via-orange-100 to-yellow-100 sm:text-2xl md:text-3xl leading-snug">
              STATION DETONATOR REGISTER (APPENDIX VII)
            </h1>
            <div className="mx-auto mt-5 h-1 w-20 rounded-full bg-linear-to-r from-amber-400 to-orange-500" />
          </header>

          <div className="space-y-5 px-5 py-8 sm:px-10 sm:py-10">
            <section className="rounded-2xl border border-slate-600/40 bg-slate-900/40 p-4 ring-1 ring-white/4 sm:p-5">
              <h2 className="text-base font-semibold text-amber-100 sm:text-lg">Overview</h2>
              <p className="mt-4 rounded-2xl border border-slate-600/40 bg-slate-900/40 p-4 text-[15px] leading-[1.75] text-slate-300 ring-1 ring-white/4 sm:p-5 sm:text-base">
                The station detonator register is an important safety document maintained at every
                railway station. It records all details related to the stock, issue, use, and return
                of detonators, as well as particulars of fog signalmen.
              </p>
            </section>

            <ol className="space-y-3">
              {overviewPoints.map((item) => (
                <li
                  key={item.number}
                  className="flex gap-3 rounded-2xl border border-slate-600/40 bg-slate-900/40 p-4 ring-1 ring-white/4 transition-colors hover:border-amber-500/30 hover:bg-slate-900/60 sm:p-5"
                  style={{ animation: `fade-up 0.55s ease-out ${0.04 * Number(item.number)}s both` }}
                >
                  <span className="mt-0.5 inline-flex h-7 min-w-9 shrink-0 items-center justify-center rounded-lg bg-linear-to-br from-amber-400/25 to-orange-600/25 px-1.5 text-xs font-bold text-amber-100 ring-1 ring-amber-400/35">
                    {item.number}
                  </span>
                  <p className="text-[15px] leading-[1.75] text-slate-300 sm:text-base">
                    <span className="font-semibold text-amber-100/90">{item.title} </span>
                    {item.text}
                  </p>
                </li>
              ))}

              <li className="rounded-2xl border border-slate-600/40 bg-slate-900/40 p-4 ring-1 ring-white/4 sm:p-5">
                <div className="flex gap-3">
                  <span className="mt-0.5 inline-flex h-7 min-w-9 shrink-0 items-center justify-center rounded-lg bg-linear-to-br from-amber-400/25 to-orange-600/25 px-1.5 text-xs font-bold text-amber-100 ring-1 ring-amber-400/35">
                    6
                  </span>
                  <div>
                    <p className="text-[15px] leading-[1.75] text-slate-300 sm:text-base">
                      <span className="font-semibold text-amber-100/90">Structure of Register:</span>
                    </p>
                    <p className="mt-1 text-[15px] leading-[1.75] text-slate-300 sm:text-base">
                      The register has four parts:
                    </p>
                    <ArrowList items={registerParts} />
                  </div>
                </div>
              </li>

              {closingPoints.map((item) => (
                <li
                  key={item.number}
                  className="flex gap-3 rounded-2xl border border-slate-600/40 bg-slate-900/40 p-4 ring-1 ring-white/4 transition-colors hover:border-amber-500/30 hover:bg-slate-900/60 sm:p-5"
                >
                  <span className="mt-0.5 inline-flex h-7 min-w-9 shrink-0 items-center justify-center rounded-lg bg-linear-to-br from-amber-400/25 to-orange-600/25 px-1.5 text-xs font-bold text-amber-100 ring-1 ring-amber-400/35">
                    {item.number}
                  </span>
                  <p className="text-[15px] leading-[1.75] text-slate-300 sm:text-base">
                    <span className="font-semibold text-amber-100/90">{item.title} </span>
                    {item.text}
                  </p>
                </li>
              ))}
            </ol>
          </div>
        </article>

        <div className="mt-10 flex justify-center">
          <Link
            href="/topics/operating-material"
            className="group inline-flex items-center gap-2 rounded-full border border-amber-400/25 bg-slate-900/70 px-4 py-2 text-sm font-medium text-amber-100 shadow-lg shadow-black/15 ring-1 ring-amber-500/10 transition hover:border-amber-300/45 hover:bg-slate-900/90"
          >
            <span className="transition-transform group-hover:-translate-x-0.5" aria-hidden>
              ←
            </span>
            <span>Back to Operating Material</span>
          </Link>
        </div>
      </div>

      <style jsx>{`
        @keyframes fade-up {
          from {
            opacity: 0;
            transform: translateY(18px);
          }
          to {
            opacity: 1;
            transform: translateY(0);
          }
        }
        @keyframes soft-pulse {
          0%,
          100% {
            transform: scale(1);
            opacity: 1;
          }
          50% {
            transform: scale(1.04);
            opacity: 0.92;
          }
        }
      `}</style>
    </div>
  )
}

export default StationDetonatorRegisterPage
