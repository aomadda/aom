...
const operatingManualTopics = [
  {
    ...
    subtitle: 'Block period & output',
    description:
      'Track maintenance machines for AOM exam — tamping, ballast cleaning, track renewal, block timings, output, and speed restrictions.',
    icon: Hammer,
      ...
      <div className="relative mx-auto max-w-7xl">
        ...
        <div className="rounded-3xl border border-indigo-500/30 bg-slate-900/70 px-8 py-8 text-center shadow-xl shadow-black/20 ring-1 ring-indigo-500/15 backdrop-blur-sm sm:px-12 sm:py-10">
          <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl border border-white/10 bg-white/5">
            <BookOpen className="h-7 w-7 text-indigo-200" strokeWidth={1.75} />
          </div>
          ...
          <p className="mx-auto mt-4 max-w-2xl text-sm leading-relaxed text-slate-300 sm:text-base">
            Exam-oriented Operating Manual notes — station working, train operations, and track machine working.
          </p>
        </div>

        <div className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {operatingManualTopics.map((topic) => {
