...
export default function LHBQuiz() {
  ...
  const rawQuestions = ((establishmentRulesQuizzes.quizzes as Record<string, QuizQuestion[]>)['lhb']) || [];
  ...
  if (showResults) {
    ...
    return (
      <div className="min-h-screen bg-linear-to-br from-orange-50 to-orange-100 py-8 sm:py-12 px-3 sm:px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl p-6 sm:p-8 mb-6 sm:mb-8">
            <div className="text-center mb-6 sm:mb-8">
              <div className="inline-flex items-center justify-center w-24 h-24 sm:w-32 sm:h-32 bg-linear-to-br from-orange-500 to-orange-600 rounded-full mb-4 sm:mb-6">
                <div className="text-center">
                  <div className="text-2xl sm:text-3xl font-bold text-white">{percentage}%</div>
                  <div className="text-xs sm:text-sm text-orange-100">Score</div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
              ...
              <div className="bg-orange-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 text-center border border-orange-200">
                ...
                <div className="text-2xl sm:text-3xl font-bold text-orange-600 mb-1 sm:mb-2">{questions.length}</div>
                <div className="text-orange-700 font-medium text-xs sm:text-sm lg:text-base">Total Questions</div>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
              <button
                ...
                className="flex-1 bg-linear-to-r from-orange-600 to-orange-700 text-white py-2 sm:py-3 px-4 sm:px-6 rounded-full hover:from-orange-700 hover:to-orange-800 transition-all duration-200 font-medium shadow-lg text-sm sm:text-base"
              >
  ...
  if (questions.length === 0) {
    return (
      <div className="min-h-screen bg-linear-to-br from-orange-50 to-orange-100 py-8 sm:py-12 px-3 sm:px-4 flex items-center justify-center">
        <div className="text-center">
          <p className="text-gray-600 mb-4">No questions available for LHB quiz yet.</p>
          <Link href="/quizzes/establishment-rules" className="inline-flex items-center px-6 py-3 bg-orange-600 text-white rounded-lg font-semibold hover:bg-orange-700 transition-colors">
            Back to Establishment Rules
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-orange-50 to-orange-100 py-6 sm:py-8 px-3 sm:px-4">
      <div className="max-w-6xl mx-auto">
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6 lg:p-8">
          <div className="text-center mb-6 sm:mb-8">
            ...
            <div className="flex items-center justify-center text-gray-600 mb-3 sm:mb-4">
              <span className="bg-orange-100 text-orange-800 px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium">Quiz</span>
            ...
            <div className="w-full bg-gray-200 rounded-full h-1.5 sm:h-2">
              <div className="bg-linear-to-r from-orange-500 to-orange-600 h-1.5 sm:h-2 rounded-full transition-all duration-300" style={{ width: `${((currentQuestion + 1) / questions.length) * 100}%` }}></div>
            </div>
          </div>
            ...
            <div className="space-y-2 sm:space-y-3">
              {currentQ.options.map((option: string, index: number) => {
                ...
                if (isSelected) {
                  ...
                  optionClasses += 'border-gray-300 hover:border-orange-300 hover:bg-orange-50 hover:shadow-sm';
                }
                return (
                  <button
                    key={index}
                    ...
                    <div className="flex items-center">
                      <div className={`w-5 h-5 sm:w-6 sm:h-6 rounded-full border-2 mr-3 sm:mr-4 flex items-center justify-center ${
                        isSelected ? (isCorrectAnswer ? 'border-green-500 bg-green-500' : 'border-red-500 bg-red-500')
                          : showExplanation && isCorrectAnswer ? 'border-green-500 bg-green-500' : 'border-gray-300 group-hover:border-orange-400'
                      }`}>
                        {(isSelected || (showExplanation && isCorrectAnswer)) ? (
                          ...
                          <span className="text-xs font-medium text-gray-400 group-hover:text-orange-400">{String.fromCharCode(65 + index)}</span>
                        )}
                      </div>
          ...
          <div className="flex justify-between items-center">
            ...
            <button
              ...
              className="px-6 sm:px-8 py-2 sm:py-3 bg-linear-to-r from-orange-600 to-orange-700 text-white rounded-md sm:rounded-lg hover:from-orange-700 hover:to-orange-800 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 font-medium shadow-lg flex items-center text-sm sm:text-base"
            >
