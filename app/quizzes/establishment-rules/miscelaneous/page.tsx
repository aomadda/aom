...
export default function MiscellaneousQuiz() {
  const router = useRouter();
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [quizStartTime, setQuizStartTime] = useState<number>(0);

  // Get the category data - filter out empty questions
  const rawQuestions = ((establishmentRulesQuizzes.quizzes as Record<string, QuizQuestion[]>)['miscelaneous']) || [];
  const questions = rawQuestions.filter((q) => q.question && q.question.trim() !== '');

  const categoryData = {
    title: 'Miscellaneous',
    description: 'Test your knowledge of miscellaneous establishment rules - IREM, UMID portal, and more',
    color: 'from-lime-500 to-lime-600',
  };

  const currentQuizId = 'miscelaneous';
  ...
  if (showResults) {
    ...
    return (
      <div className="min-h-screen bg-linear-to-br from-lime-50 to-lime-100 py-8 sm:py-12 px-3 sm:px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl p-6 sm:p-8 mb-6 sm:mb-8">
            <div className="text-center mb-6 sm:mb-8">
              <div className="inline-flex items-center justify-center w-24 h-24 sm:w-32 sm:h-32 bg-linear-to-br from-lime-500 to-lime-600 rounded-full mb-4 sm:mb-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
              ...
              <div className="bg-lime-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 text-center border border-lime-200">
                ...
                <div className="text-2xl sm:text-3xl font-bold text-lime-600 mb-1 sm:mb-2">{questions.length}</div>
                <div className="text-lime-700 font-medium text-xs sm:text-sm lg:text-base">Total Questions</div>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
              <button
                ...
                className="flex-1 bg-linear-to-r from-lime-600 to-lime-700 text-white py-2 sm:py-3 px-4 sm:px-6 rounded-full hover:from-lime-700 hover:to-lime-800 transition-all duration-200 font-medium shadow-lg text-sm sm:text-base"
              >
  ...
  if (questions.length === 0) {
    return (
      <div className="min-h-screen bg-linear-to-br from-lime-50 to-lime-100 py-8 sm:py-12 px-3 sm:px-4 flex items-center justify-center">
        <div className="text-center">
          <p className="text-gray-600 mb-4">No questions available for Miscellaneous quiz yet.</p>
          <Link href="/quizzes/establishment-rules" className="inline-flex items-center px-6 py-3 bg-lime-600 text-white rounded-lg font-semibold hover:bg-lime-700 transition-colors">
            Back to Establishment Rules
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-lime-50 to-lime-100 py-6 sm:py-8 px-3 sm:px-4">
      <div className="max-w-6xl mx-auto">
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6 lg:p-8">
          <div className="text-center mb-6 sm:mb-8">
            ...
            <div className="flex items-center justify-center text-gray-600 mb-3 sm:mb-4">
              <span className="bg-lime-100 text-lime-800 px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium">Quiz</span>
            ...
            <div className="w-full bg-gray-200 rounded-full h-1.5 sm:h-2">
              <div className="bg-linear-to-r from-lime-500 to-lime-600 h-1.5 sm:h-2 rounded-full transition-all duration-300" style={{ width: `${((currentQuestion + 1) / questions.length) * 100}%` }}></div>
            </div>
          </div>
            ...
            <div className="space-y-2 sm:space-y-3">
              {currentQ.options.map((option: string, index: number) => {
                ...
                if (isSelected) {
                  ...
                  optionClasses += 'border-gray-300 hover:border-lime-300 hover:bg-lime-50 hover:shadow-sm';
                }
                return (
                  <button
                    key={index}
                    ...
                    <div className="flex items-center">
                      <div className={`w-5 h-5 sm:w-6 sm:h-6 rounded-full border-2 mr-3 sm:mr-4 flex items-center justify-center ${
                        isSelected ? (isCorrectAnswer ? 'border-green-500 bg-green-500' : 'border-red-500 bg-red-500')
                          : showExplanation && isCorrectAnswer ? 'border-green-500 bg-green-500' : 'border-gray-300 group-hover:border-lime-400'
                      }`}>
                        {(isSelected || (showExplanation && isCorrectAnswer)) ? (
                          ...
                          <span className="text-xs font-medium text-gray-400 group-hover:text-lime-400">{String.fromCharCode(65 + index)}</span>
                        )}
                      </div>
          ...
          <div className="flex justify-between items-center">
            ...
            <button
              ...
              className="px-6 sm:px-8 py-2 sm:py-3 bg-linear-to-r from-lime-600 to-lime-700 text-white rounded-md sm:rounded-lg hover:from-lime-700 hover:to-lime-800 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 font-medium shadow-lg flex items-center text-sm sm:text-base"
            >
