...
const CATEGORY_ID = 'block-working-manual'
const CATEGORY_TITLE = 'Block Working Manual'
const CATEGORY_COLOR = 'from-teal-500 to-cyan-600'
const CATEGORY_QUIZZES = blockWorkingManualQuizzes.quizzes
const RESULTS_STORAGE_KEY = 'bwm_quiz_results'
const SESSION_STORAGE_KEY = 'bwm_quiz_session'
...
export default function BlockWorkingManualQuiz() {
    ...
    ;(async () => {
      ...
      if (candidate?.quizId) {
        ...
        if (quizQuestions.length > 0 && candidate.userAnswers.length === quizQuestions.length) {
          // Keep progress ready for Resume Quiz — do not auto-enter.
  ...
  useEffect(() => {
    if (!sessionReady || !quizActive || showResults || questions.length === 0 || isCurrentCompleted)
      return
  ...
  useEffect(() => {
    if (!sessionReady || !quizActive || showResults || isCurrentCompleted || questions.length === 0)
      return
          ...
          <div className="mb-6 rounded-2xl bg-white p-6 shadow-2xl sm:mb-8 sm:rounded-3xl sm:p-8">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:gap-4">
              <button
                ...
                className="flex-1 rounded-full bg-linear-to-r from-teal-600 to-teal-700 px-4 py-2 text-sm font-medium text-white shadow-lg transition-all duration-200 hover:from-teal-700 hover:to-teal-800 hover:shadow-xl sm:px-6 sm:py-3 sm:text-base"
              >
                Back to Quizzes
              </button>
              <button
                ...
                return (
                  ...
                  >
                    ...
                    {question.explanation ? (
                      <div className="mt-3 rounded-2xl border border-sky-100 bg-linear-to-br from-sky-50 to-indigo-50 p-4">
