...
import { blockWorkingManualQuizzes } from '@/assets/block-working-manual';
...
export default function BlockWorkingManualQuiz() {
  ...
  const categoryData = {
    ...
    description: 'Test your knowledge of Block Working Manual for Indian Railways',
    color: 'from-teal-500 to-cyan-600',
    quizzes: blockWorkingManualQuizzes.quizzes
  };
      ...
      try {
        ...
        if (response.ok) {
          ...
          const categoryCompletedQuizzes = data.recentActivity
            ?.filter((quiz: { categoryId: string; quizId: string }) => quiz.categoryId === 'block-working-manual')
                ...
                {Object.keys(categoryData.quizzes).map((quizId) => {
                  ...
                  return (
                    <div key={quizId} className={`bg-linear-to-br from-gray-50 to-gray-100 rounded-lg py-6 lg:px-6 px-2 border-2 transition-all duration-300 transform hover:scale-105 ${
                        isCurrentQuiz ? 'border-teal-400 shadow-lg' : 'border-gray-400 hover:shadow-lg'
                      }`}>
