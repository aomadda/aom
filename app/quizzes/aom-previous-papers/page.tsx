...
import { aomPreviousPapersQuizzes } from '@/assets/aom-previous-papers/aom-previous-papers';
...
export default function AomPreviousPapersQuiz() {
  ...
  const categoryData = {
    ...
    description: 'Test your knowledge of AOM Previous Papers for Indian Railways',
      ...
      try {
        ...
        if (response.ok) {
          ...
          const categoryCompletedQuizzes = data.recentActivity
            ?.filter((quiz: { categoryId: string; quizId: string }) => quiz.categoryId === 'aom-previous-papers')
  ...
  const updateUserProgress = async (finalScore: number, correctAnswers: number) => {
    try {
      ...
      const response = await fetch('/api/progress', {
        ...
        body: JSON.stringify({
          categoryId: 'aom-previous-papers',
