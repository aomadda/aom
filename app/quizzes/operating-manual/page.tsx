...
import { operatingManualQuizzes } from '@/assets/operating-manual';
...
export default function OperatingManualQuiz() {
  ...
  const categoryData = {
    ...
    description: 'Test your knowledge of Operating Manual for Indian Railways',
    color: 'from-blue-500 to-indigo-600',
    quizzes: operatingManualQuizzes.quizzes || {}
  };
  
  const quizIds = Object.keys(categoryData.quizzes);
  const [currentQuizId, setCurrentQuizId] = useState(quizIds.length > 0 ? quizIds[0] : '');
  
  // Get the current quiz questions
  const currentQuiz = currentQuizId ? (categoryData.quizzes[currentQuizId as keyof typeof categoryData.quizzes] as QuizQuestion[]) : [];
  const questions: QuizQuestion[] = currentQuiz || [];
      ...
      try {
        ...
        if (response.ok) {
          ...
          const categoryCompletedQuizzes = data.recentActivity
            ?.filter((quiz: { categoryId: string; quizId: string }) => quiz.categoryId === 'operating-manual')
  ...
  const handleQuizSelect = (quizId: string) => {
    const selectedQuiz = (categoryData.quizzes[quizId as keyof typeof categoryData.quizzes] as QuizQuestion[]) || [];
  ...
  if (quizIds.length === 0 || questions.length === 0) {
    return (
      <div className="min-h-screen bg-linear-to-br from-blue-50 to-indigo-50 py-8 sm:py-12 px-3 sm:px-4">
        <div className="max-w-6xl mx-auto">
          <div className="bg-white rounded-2xl shadow-2xl p-8 text-center">
            <div className="text-6xl mb-4">📘</div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 mb-4">Operating Manual</h1>
            <p className="text-gray-600 mb-6">Quiz data is not available yet. Please check back later.</p>
            <Link 
              href="/quizzes"
              className="inline-flex items-center px-6 py-3 bg-linear-to-r from-blue-600 to-indigo-600 text-white rounded-lg font-semibold hover:from-blue-700 hover:to-indigo-700 transition-all duration-200 shadow-lg hover:shadow-xl"
            >
      ...
      <div className="min-h-screen bg-linear-to-br from-blue-50 to-indigo-50 py-8 sm:py-12 px-3 sm:px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl p-6 sm:p-8 mb-6 sm:mb-8">
            {/* Score Display */}
            <div className="text-center mb-6 sm:mb-8">
              <div className="inline-flex items-center justify-center w-24 h-24 sm:w-32 sm:h-32 bg-linear-to-br from-blue-500 to-indigo-600 rounded-full mb-4 sm:mb-6">   
          ...
          <div className="bg-white rounded-xl shadow-2xl p-8">
            ...
            <div className="space-y-4">
              {questions.map((question: { question: string; options: string[]; correct: number; explanation?: string }, index: number) => {
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-blue-50 to-indigo-50 py-6 sm:py-8 px-3 sm:px-4">
          ...
          <div className="text-center mb-6 sm:mb-8">
            ...
            <div className="flex items-center justify-center text-gray-600 mb-3 sm:mb-4">
              <span className="bg-blue-100 text-blue-800 px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium">
                {currentQuizId ? currentQuizId.replace('chapter-', 'Chapter ').replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()) : 'Quiz'}
              </span>
            ...
            <div className="w-full bg-gray-200 rounded-full h-1.5 sm:h-2">
              <div 
                className="bg-linear-to-r from-blue-500 to-indigo-600 h-1.5 sm:h-2 rounded-full transition-all duration-300 ease-out"
