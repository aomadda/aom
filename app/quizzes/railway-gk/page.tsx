...
import { railwayGkQuizzes } from '@/assets/railway-gk';
...
export default function RailwayGKQuiz() {
  ...
  const categoryData = {
    title: 'Railway GK',
    description: 'Test your knowledge of Indian Railways General Knowledge',
    color: 'from-orange-500 to-amber-600',
    quizzes: railwayGkQuizzes.quizzes
  };
  
  const [currentQuizId, setCurrentQuizId] = useState('chapter-1'); // Start with first quiz
  
  // Get the current quiz questions
  const currentQuiz = categoryData.quizzes[currentQuizId as keyof typeof categoryData.quizzes] as QuizQuestion[];
      ...
      try {
        ...
        if (response.ok) {
          ...
          const categoryCompletedQuizzes = data.recentActivity
            ?.filter((quiz: { categoryId: string; quizId: string }) => quiz.categoryId === 'railway-gk')
  ...
  const handleNextQuiz = () => {
    const quizIds = Object.keys(categoryData.quizzes).filter(quizId => {
      const quiz = categoryData.quizzes[quizId as keyof typeof categoryData.quizzes] as QuizQuestion[];
    ...
    if (currentIndex < quizIds.length - 1) {
      ...
      const nextQuiz = categoryData.quizzes[nextQuizId as keyof typeof categoryData.quizzes] as QuizQuestion[];
      setCurrentQuestion(0);
  ...
  if (showResults) {
    ...
    return (
      <div className="min-h-screen bg-linear-to-br from-orange-50 to-amber-50 py-8 sm:py-12 px-3 sm:px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl p-6 sm:p-8 mb-6 sm:mb-8">
            {/* Score Display */}
            <div className="text-center mb-6 sm:mb-8">
              <div className="inline-flex items-center justify-center w-24 h-24 sm:w-32 sm:h-32 bg-linear-to-br from-orange-500 to-amber-600 rounded-full mb-4 sm:mb-6">   
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
              <button
                ...
                className="flex-1 bg-linear-to-r from-orange-600 to-orange-700 text-white py-2 sm:py-3 px-4 sm:px-6 rounded-full sm:rounded-full hover:from-orange-700 hover:to-orange-800 transition-all duration-200 font-medium shadow-lg hover:shadow-xl text-sm sm:text-base"
              >
                ...
                {Object.keys(categoryData.quizzes).map((quizId) => {
                  ...
                  return (
                    <div key={quizId} className={`bg-linear-to-br from-gray-50 to-gray-100 rounded-lg py-6 lg:px-6 px-2 border-2 transition-all duration-300 transform hover:scale-105 ${
                        isCurrentQuiz ? 'border-orange-400 shadow-lg' : 'border-gray-400 hover:shadow-lg'
                      }`}>
                      ...
                      <p className="text-gray-600 text-sm mb-4">{(categoryData.quizzes[quizId as keyof typeof categoryData.quizzes] as QuizQuestion[]).length} Questions</p>
                      <button 
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-orange-50 to-amber-50 py-6 sm:py-8 px-3 sm:px-4">
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6 lg:p-8">
          ...
          <div className="text-center mb-6 sm:mb-8">
            ...
            <div className="w-full bg-gray-200 rounded-full h-1.5 sm:h-2">
              <div 
                className="bg-linear-to-r from-orange-500 to-amber-600 h-1.5 sm:h-2 rounded-full transition-all duration-300 ease-out"
          ...
          <div className="flex justify-between items-center">
            ...
            <button
              ...
              className="px-6 sm:px-8 py-2 sm:py-3 bg-linear-to-r from-orange-600 to-amber-600 text-white rounded-md sm:rounded-lg hover:from-orange-700 hover:to-amber-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 font-medium shadow-lg hover:shadow-xl flex items-center text-sm sm:text-base"
            >
