...
import { chemistryQuiz } from '@/assets/general-awareness/quizzes/chemistry';
...
export default function ChemistryQuizPage() {
  ...
  const categoryData = {
    title: 'Chemistry',
    description:
      'Practice atomic structure, periodic trends, reactions, and core chemistry facts for competitive exams.',
    color: 'from-violet-600 to-fuchsia-600',
    quizzes: chemistryQuiz.quizzes,
  };
      ...
      try {
        ...
        if (response.ok) {
          ...
          const categoryCompletedQuizzes = data.recentActivity
            ?.filter((quiz: { categoryId: string; quizId: string }) => quiz.categoryId === 'general-awareness-chemistry')
  ...
  if (showResults) {
    ...
    return (
      <div className="min-h-screen bg-linear-to-br from-violet-50 to-fuchsia-50 py-8 sm:py-12 px-3 sm:px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl p-6 sm:p-8 mb-6 sm:mb-8">
            <div className="text-center mb-6 sm:mb-8">
              <div className="inline-flex items-center justify-center w-24 h-24 sm:w-32 sm:h-32 bg-linear-to-br from-violet-600 to-fuchsia-600 rounded-full mb-4 sm:mb-6 shadow-lg ring-4 ring-white/80">
                <div className="text-center">
                  <div className="text-2xl sm:text-3xl font-bold text-white">{percentage}%</div>
                  <div className="text-xs sm:text-sm text-violet-100">Score</div>
                </div>
              </div>
              ...
              <p className="text-gray-500 text-xs sm:text-sm mt-2">
                {categoryData.title} — {formatQuizLabel(currentQuizId)}
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
              ...
              <div className="bg-violet-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 text-center border border-violet-200">
                <div className="w-12 h-12 sm:w-16 sm:h-16 bg-violet-500 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4">
                  <svg className="w-6 h-6 sm:w-8 sm:h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path
                      strokeLinecap="round"
                ...
                <div className="text-2xl sm:text-3xl font-bold text-violet-700 mb-1 sm:mb-2">{questions.length}</div>
                <div className="text-violet-800 font-medium text-xs sm:text-sm lg:text-base">Total Questions</div>
              </div>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    {question.explanation && (
                      <div className="text-sm text-fuchsia-900 mt-2 p-2 bg-violet-50 rounded border border-violet-100">
            ...
            {loadingProgress ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {Object.keys(categoryData.quizzes).map((quizId) => {
                  ...
                  const quizNum = quizId.replace(/^quiz-/i, '');

                  return (
                    <div
                      key={quizId}
                      className={`bg-linear-to-br from-gray-50 to-gray-100 rounded-lg py-6 lg:px-6 px-2 border-2 transition-all duration-300 transform hover:scale-105 ${
                        isCurrentQuiz ? 'border-violet-400 shadow-lg' : 'border-gray-400 hover:shadow-lg'
                      }`}
                    >
                      <div className="flex items-center justify-between mb-4">
                        <div className={`lg:w-12 lg:h-12 w-8 h-8 bg-linear-to-br ${categoryData.color} rounded-full flex items-center justify-center`}>
                          <span className="text-white lg:text-lg text-sm font-bold">{quizNum}</span>
                        </div>
                        {isCompleted && (
                      ...
                      <h4 className="text-lg font-bold text-gray-700 mb-2">{formatQuizLabel(quizId)}</h4>
                      <p className="text-gray-600 text-sm mb-4">
                        {categoryData.quizzes[quizId as keyof typeof categoryData.quizzes].length} Questions
                      </p>
                      <button
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-violet-50 to-fuchsia-50 py-6 sm:py-8 px-3 sm:px-4">
      <div className="max-w-6xl mx-auto">
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6 lg:p-8">
          <div className="text-center mb-6 sm:mb-8">
            ...
            <div className="flex items-center justify-center text-gray-600 mb-3 sm:mb-4 flex-wrap gap-x-2 gap-y-1">
              <span className="bg-violet-100 text-fuchsia-900 px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium">
            ...
            <div className="w-full bg-gray-200 rounded-full h-1.5 sm:h-2">
              <div
                className="bg-linear-to-r from-violet-500 to-fuchsia-600 h-1.5 sm:h-2 rounded-full transition-all duration-300 ease-out"
            ...
            <div className="space-y-2 sm:space-y-3">
              {currentQ.options.map((option, index) => {
                ...
                if (isSelected) {
                  ...
                  optionClasses += ' border-gray-300 hover:border-violet-300 hover:bg-violet-50/80 hover:shadow-sm';
                }
                    ...
                    <div className="flex items-center">
                      ...
                      >
                        {isSelected || (showExplanation && isCorrectAnswer) ? (
                          ...
                          <span className="text-xs font-medium text-gray-400 group-hover:text-violet-700">{String.fromCharCode(65 + index)}</span>
                        )}
                      </div>
          ...
          {showExplanation && (
            <div
              className={`mb-4 sm:mb-6 p-3 sm:p-4 rounded-md sm:rounded-lg border ${
                isCorrect ? 'bg-green-50 border-green-500 text-green-700' : 'bg-red-50 border-rose-500 text-rose-700'
              }`}
            >
          ...
          <div className="flex justify-between items-center gap-3">
            ...
            <button
              ...
              className="px-6 sm:px-8 py-2 sm:py-3 bg-linear-to-r from-violet-600 to-fuchsia-600 text-white rounded-md sm:rounded-lg hover:from-violet-700 hover:to-fuchsia-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 font-medium shadow-lg hover:shadow-xl flex items-center text-sm sm:text-base"
            >
              {currentQuestion === questions.length - 1 ? 'Finish Quiz' : 'Next Question'}
              <svg className="w-3 h-3 sm:w-4 sm:h-4 ml-1 sm:ml-2 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
              </svg>
            </button>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {Object.keys(categoryData.quizzes).map((quizId) => {
                ...
                return (
                  <div
                    key={quizId}
                    className={`bg-linear-to-br from-gray-50 to-gray-100 rounded-lg py-6 lg:px-6 px-3 border-2 transition-all duration-300 transform hover:scale-105 ${
                      isCurrentQuiz ? 'border-violet-400 shadow-lg' : 'border-gray-400 hover:shadow-lg'
                    }`}
                  >
                    <div className="flex items-center justify-between mb-4">
