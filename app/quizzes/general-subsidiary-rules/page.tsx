...
import { GENERAL_SUBSIDIARY_RULES_QUIZ_CHAPTERS } from '@/lib/general-subsidiary-rules-quiz-chapters'

const GeneralSubsidiaryRulesQuizzesPage = () => {
  const pathname = usePathname()

  return (
    <div className="min-h-screen bg-linear-to-br from-blue-50 via-sky-50 to-indigo-50 py-8 sm:py-12">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-8 text-center sm:mb-12">
          <h1 className="mb-4 bg-linear-to-r from-blue-600 via-sky-600 to-indigo-600 bg-clip-text text-3xl font-bold text-transparent sm:mb-6 sm:text-4xl md:text-5xl lg:text-6xl">
          ...
          <div className="mx-auto max-w-3xl rounded-xl border border-white/20 bg-white/80 p-4 shadow-lg backdrop-blur-sm sm:rounded-2xl sm:p-6">
            <p className="mb-3 text-sm text-gray-600 sm:mb-4 sm:text-base md:text-lg">
              Practice General & Subsidiary Rules questions chapter-wise in exam mode
            </p>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 sm:gap-6 lg:gap-8">
          {GENERAL_SUBSIDIARY_RULES_QUIZ_CHAPTERS.map((chapter) => {
            const href = `/quizzes/general-subsidiary-rules/${chapter.slug}`
          ...
          <div className="inline-block rounded-xl border border-white/20 bg-white/60 p-6 shadow-lg backdrop-blur-sm">
            <p className="text-sm text-gray-600">
              <span className="font-semibold text-blue-600">
                {GENERAL_SUBSIDIARY_RULES_QUIZ_CHAPTERS.length}
              </span>{' '}
              General & Subsidiary Rules quizzes available
            </p>
          </div>
        </div>
      </div>
...
export default GeneralSubsidiaryRulesQuizzesPage
