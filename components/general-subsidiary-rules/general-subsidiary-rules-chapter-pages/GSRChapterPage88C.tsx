...
const GSRChapterPage88C = () => {
  const introText = `3.81 Duties of Loco Pilot when a departure Stop signal is &apos;on&apos; or defective.—`
  
  const item1Text = `The Loco Pilot of a train shall not pass a departure Stop signal that refers to him, when it is &apos;on&apos; or defective, unless his train has been brought to a stop at the station where the defective signal is situated and he is authorised to do so —`
  
  const item1aText = `by a written permission from the Station Master, in addition, in the case of a Starter or Advanced Starter protecting points, he shall not pass such signals, when &apos;on&apos; or defective, unless he also receives a Proceed hand signal from a duly authorised member of the station staff posted at the signal, or`
  
  const item1bText = `by taking &apos;off&apos; the Calling-on signal, if provided under special instructions, vide sub-rule (2) of Rule 3.13. (Item no. 8 of AS-11 dt. 22.08.2023)`
  
  const item2Text = `In the case of a last Stop signal, he shall not pass such signal, when &apos;on&apos; or defective, unless he is also in possession of a proper authority to proceed under the system of working.`
