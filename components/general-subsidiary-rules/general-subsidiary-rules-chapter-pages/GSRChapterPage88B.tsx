...
const GSRChapterPage88B = () => {
  const introText = `3.80. Duties of Loco Pilot when an approach Stop signal is &apos;on&apos; or defective.—`
  
  const item1Text = `The Loco Pilot of a train shall not pass an Outer, a Home or a Routing signal that refers to him, when it is &apos;on&apos; or defective, unless-`
  
  const item1aText = `he has, at a previous station, received notice in writing specifying that the signal is out of order and unless he also receives a Proceed hand signal from a railway servant in uniform at the foot of such signal; or`
  
  const item1bText = `after coming to a stand, he is either given a written authority by the Station Master to proceed past such signal or is authorised by a Calling-on signal in the &apos;off&apos; position or is authorised by the Station Master over the signal post telephone in accordance with special instructions.`
  
  const item2Text = `The Loco Pilot of a train while passing an Outer, a Home or a Routing signal, when it is &apos;on&apos; or defective, shall ensure that the speed of his train does not exceed 15 kilometres an hour.`
...
export default GSRChapterPage88B
