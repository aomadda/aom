...
import { generalRules } from '@/assets/general-rules/general-rules'
...
function computeDepth(
  ...
  if (kind === 'number') {
    ...
    if (lastNumber) {
      ...
      // Continue a nested sequence: (54) → (1) → (2)
      if (lastNumber.depth > 0 && n === prev + 1) return lastNumber.depth
      // Start nesting under a higher clause: (54) → (1)
...
function parseDescription(description: string): DescriptionPart[] {
  ...
  // Split only at real list markers (line-start, or after an em/en dash),
  // so values like "(24 of 1989)" or "(if any)" stay inside the text.
  const parts = normalized
    .split(
      /(?=(?:^|\n)\s*(?:\(\d+\)|\((?:i|ii|iii|iv|v|vi|vii|viii|ix|x|xi|xii|xiii|xiv|xv)\)|\([a-z]\)|[a-z]\.)|(?<=[–—])\s*\([a-z]\))/i,
    )
    .map((part) => part.trim())
    .filter(Boolean)
      ...
      {parts.map((part, index) => {
        ...
        return (
          ...
          >
            ...
            <div
              className={`relative overflow-hidden rounded-xl border px-3.5 py-3 shadow-sm shadow-black/25 transition-all duration-300 ${styles.card}`}
            >
              <div
...
function flattenRules(): FlatRule[] {
  const items: FlatRule[] = []
  let number = 0
      ...
      <div className="relative z-10 container mx-auto px-4 py-6">
        <div className="text-center mb-12">
          ...
          <div className="mt-6 flex items-center justify-center space-x-4 text-gray-400">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm">{allRules.length} Rules</span>
            </div>
          </div>
        </div>
        ...
        <div className="sticky top-20 z-40 mb-8">
          <div className="max-w-2xl mx-auto">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search by Chapter, Title, Rule or Description"
                value={searchTerm}
        ...
        <div className="text-center mb-8">
          <p className="text-gray-400">
            Showing <span className="text-white font-semibold">{filteredRules.length}</span> of{' '}
            <span className="text-white font-semibold">{allRules.length}</span> rules
          </p>
        </div>
            ...
            >
              ...
              <div className="mt-8 space-y-4 flex-1">
                ...
                <div>
                  ...
                  <h3 className="mt-1 text-base font-medium text-white leading-tight">
                    {item.title}
                  </h3>
                </div>

                <div>
                  <span className="text-xs font-semibold uppercase tracking-wide text-green-400">Rule</span>
                  <p className="mt-1 text-sm font-semibold text-cyan-300 tracking-wide">{item.rule}</p>
                </div>
        ...
        {filteredRules.length === 0 ? (
          <div className="text-center py-4">
            ...
            <h3 className="lg:text-2xl text-lg font-bold text-white mb-2">No rules found</h3>
