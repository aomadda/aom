...
import {
  tigerReservesByState,
  tigerReservesTitleLine,
} from '@/assets/general-awareness/tiger-reserves'

type TigerReservesProps = {
  showBackNav?: boolean
}

const totalReserveCount = tigerReservesByState.reduce(
    ...
    >
      ...
      <div className="relative flex items-center gap-2 sm:gap-3">
        ...
        <input
          ...
          placeholder="Tiger Reserves of India"
...
const TigerReserves = ({ showBackNav = false }: TigerReservesProps) => {
    ...
    <div className="min-h-screen bg-linear-to-br from-slate-900 via-slate-800 to-emerald-950/90 py-10 px-3 sm:px-6 lg:px-8">
        ...
        >
          <div className="rounded-2xl border border-slate-600/50 bg-slate-900/92 p-1.5 shadow-[0_12px_40px_-8px_rgba(0,0,0,0.65)] backdrop-blur-xl sm:p-2">
            <SearchBarField
              value={search}
              onChange={setSearch}
              inputId="tiger-reserves-search-floating"
              variant="floating"
            />
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div
          ...
          role="search"
          aria-label="Filter tiger reserves by state, name, or year"
        >
          <SearchBarField
            value={search}
            onChange={setSearch}
            inputId="tiger-reserves-search"
          />
          {isFiltering ? (
