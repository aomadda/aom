...
import {
  elephantReservesByState,
  elephantReservesTitleLine,
} from '@/assets/general-awareness/elephant-reserves'

type ElephantReservesProps = {
  showBackNav?: boolean
}

const totalReserveCount = elephantReservesByState.reduce(
...
function filterGroups(query: string) {
  ...
  if (!raw) {
    return elephantReservesByState
  }
  ...
  return elephantReservesByState
    .map((group) => ({
      ...
      reserves: group.reserves.filter((row) => {
        ...
        if (row.notificationYear.includes(raw)) return true
        return false
      }),
    }))
    ...
    >
      ...
      <div className="relative flex items-center gap-2 sm:gap-3">
        ...
        <input
          ...
          placeholder="Elephant Reserves of India"
...
const ElephantReserves = ({ showBackNav = false }: ElephantReservesProps) => {
    ...
    <div className="min-h-screen bg-linear-to-br from-slate-900 via-slate-800 to-emerald-950/90 py-10 px-3 sm:px-6 lg:px-8">
        ...
        >
          <div className="rounded-2xl border border-slate-600/50 bg-slate-900/92 p-1.5 shadow-[0_12px_40px_-8px_rgba(0,0,0,0.65)] backdrop-blur-xl sm:p-2">
            <SearchBarField
              value={search}
              onChange={setSearch}
              inputId="elephant-reserves-search-floating"
              variant="floating"
            />
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div
          ...
          role="search"
          aria-label="Filter elephant reserves by state, name, or year"
        >
          <SearchBarField
            value={search}
            onChange={setSearch}
            inputId="elephant-reserves-search"
          />
          {isFiltering ? (
                  ...
                  <table className="w-full min-w-[640px] text-left border-collapse">
                    <thead>
                      <tr className="border-b border-slate-600/60 bg-slate-900/40">
                        ...
                        <th
                          scope="col"
                          className="px-3 py-3 sm:px-5 text-xs font-semibold uppercase tracking-wide text-emerald-400/90 w-28 sm:w-32"
                        >
                          Year
                        </th>
                        <th
                        ...
                        >
                          Elephant Reserve Name
                        </th>
                        <th
                          scope="col"
                          className="px-3 py-3 sm:px-5 text-xs font-semibold uppercase tracking-wide text-emerald-400/90 min-w-44"
                        >
                          UNESCO Status
                        </th>
                      </tr>
