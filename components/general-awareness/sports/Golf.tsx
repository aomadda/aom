...
import {
  ...
  ryderCupSection,
  trophiesSection,
} from '@/assets/general-awareness/sports/golf'
...
const Golf = ({ showBackNav = false }: GolfProps) => {
  return (
        ...
        <div className="space-y-8 sm:space-y-10">
          <SectionShell>
            ...
            <div className="mb-2 flex items-center gap-2.5">
              ...
              <h3 className="text-base font-bold tracking-tight text-emerald-100 sm:text-lg">
                {golfOverviewSection.keyFeaturesHeading}
              </h3>
            </div>
            <ArrowBulletList className="mt-2" items={golfOverviewSection.keyFeatures} />
            ...
            <div className="mt-7 mb-2 flex items-center gap-2.5">
              ...
              <h3 className="text-base font-bold tracking-tight text-emerald-100 sm:text-lg">
                {golfOverviewSection.basicRulesHeading}
              </h3>
            </div>
            <NumberedLines lines={golfOverviewSection.basicRules} />

            <div className="mt-7 mb-2 flex items-center gap-2.5">
              <Target className="h-4.5 w-4.5 text-emerald-300" strokeWidth={2} aria-hidden />
              <h3 className="text-base font-bold tracking-tight text-emerald-100 sm:text-lg">
                {golfOverviewSection.scoringTermsHeading}
              </h3>
            </div>
            <ArrowBulletList className="mt-2" items={golfOverviewSection.scoringTerms} />
          </SectionShell>
          ...
          <SectionShell>
            <div className="flex items-start gap-3 sm:gap-4">
              ...
              <p className="min-w-0 flex-1 leading-relaxed text-slate-300">{golfIntroParagraph}</p>
            </div>
          </SectionShell>
          ...
          <SectionShell>
            <div className="mb-6 flex items-start gap-3 sm:gap-4">
              ...
              <SectionHeading className="mb-0 min-w-0 flex-1">{ryderCupSection.heading}</SectionHeading>
            </div>
            {ryderCupSection.introParagraphs.map((p, i) => (
              <p key={i} className={i === 0 ? 'leading-relaxed text-slate-300' : 'mt-3 leading-relaxed text-slate-300'}>
                {p}
              </p>
            ))}
            <p className="mt-5 leading-relaxed text-slate-300">{ryderCupSection.sinceLine}</p>
            <p className="mt-4 leading-relaxed text-slate-300">{ryderCupSection.editionLine}</p>
            <p className="mt-4 leading-relaxed text-slate-300">{ryderCupSection.placeLine}</p>
          </SectionShell>
