...
import {
  basketBallIntroParagraph,
  basketBallOverviewSection,
  basketBallPageTitle,
  famousBasketBallPlayersSection,
  fibaAsiaCupChampionshipSection,
  relatedTermsSection,
  trophiesSection,
} from '@/assets/general-awareness/sports/basketball'
...
function SubsectionTitleBar({ title }: { title: string }) {
  return (
    <h3 className="mt-6 rounded-xl border border-slate-600/45 bg-slate-900/45 px-4 py-3 text-sm font-bold uppercase tracking-wide text-emerald-300/95 sm:text-base">
      {title}
    </h3>
  )
}
...
const BasketBall = ({ showBackNav = false }: BasketBallProps) => {
  return (
        ...
        <div className="space-y-8 sm:space-y-10">
          <SectionShell>
            <div className="mb-6 flex items-start gap-3 sm:gap-4">
              ...
              <p className="min-w-0 flex-1 text-base leading-relaxed text-slate-200 sm:text-lg">
                {basketBallOverviewSection.intro}
              </p>
            </div>
            ...
            <div className="mb-2 flex items-center gap-2.5">
              ...
              <h3 className="text-base font-bold tracking-tight text-emerald-100 sm:text-lg">
                {basketBallOverviewSection.keyFeaturesHeading}
              </h3>
            </div>
            <ArrowBulletList className="mt-2" items={basketBallOverviewSection.keyFeatures} />
            ...
            <div className="mt-7 mb-2 flex items-center gap-2.5">
              ...
              <h3 className="text-base font-bold tracking-tight text-emerald-100 sm:text-lg">
                {basketBallOverviewSection.basicRulesHeading}
              </h3>
            </div>
            <NumberedLines lines={basketBallOverviewSection.basicRules} />
          </SectionShell>
          ...
          <SectionShell>
            <div className="flex items-start gap-3 sm:gap-4">
              ...
              <p className="min-w-0 flex-1 leading-relaxed text-slate-300">{basketBallIntroParagraph}</p>
            </div>
          </SectionShell>
          ...
          <SectionShell>
            <div className="mb-6 flex items-start gap-3 sm:gap-4">
              ...
              <SectionHeading className="mb-0 min-w-0 flex-1">{fibaAsiaCupChampionshipSection.heading}</SectionHeading>
            </div>
            {fibaAsiaCupChampionshipSection.introParagraphs.map((p, i) => (
              <p key={i} className={i === 0 ? 'leading-relaxed text-slate-300' : 'mt-3 leading-relaxed text-slate-300'}>
                {p}
              </p>
            ))}
            <ArrowBulletList items={fibaAsiaCupChampionshipSection.arrowBullets} />
            <SubsectionTitleBar title={fibaAsiaCupChampionshipSection.block2021Title} />
            <SubsectionTitleBar title={fibaAsiaCupChampionshipSection.block2023Title} />
          </SectionShell>
          ...
          <SectionShell>
            <div className="mb-6 flex items-start gap-3 sm:gap-4">
              ...
              <SectionHeading className="mb-0 min-w-0 flex-1">{famousBasketBallPlayersSection.heading}</SectionHeading>
            </div>
            <NumberedLines lines={famousBasketBallPlayersSection.lines} />
          </SectionShell>
        </div>
