...
import * as MP from '@/assets/general-awareness/states-information/madhya-pradesh'

type MadhyaPradeshProps = {
  showBackNav?: boolean
}
...
function ExtendedFactsGrid({
  ...
  facts: readonly MP.MadhyaPradeshExtendedFact[]
  className?: string
}) {
  return (
    <dl
...
const MadhyaPradesh = ({ showBackNav = false }: MadhyaPradeshProps) => {
  return (
      ...
      <div className="relative mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
        ...
        <div className="space-y-8 sm:space-y-9">
          <SectionShell>
            ...
            <KeyValueGrid items={MP.madhyaPradeshBasicFacts} />
            <ExtendedFactsGrid facts={MP.madhyaPradeshExtendedFacts} />
          </SectionShell>
          ...
          <SectionShell>
            <div className="mb-4 flex items-center gap-3">
              ...
              <SectionHeading>{MP.templesSectionTitle}</SectionHeading>
            </div>
            <ScrollTable
              header={
                <tr>
          ...
          <SectionShell>
            <SectionHeading>{MP.industriesSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{MP.powerPlantsSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <ExtendedFactsGrid className="mt-0" facts={[{ label: 'RIVERS', kind: 'ordered', items: MP.riversOrdered }]} />
          </SectionShell>

          <SectionShell>
            <SectionHeading>{MP.lakesSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{MP.projectsDamsSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{MP.waterfallsBeachSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
            ...
            >
              {MP.waterfallsBeachRows.map((row, i) => (
                <tr key={`${row.name}-${i}`} className="hover:bg-slate-800/40">
                  <RowNumberCell index={i} />
                  <td className="px-3 py-2.5 sm:px-4">{row.name}</td>
                  <td className="px-3 py-2.5 text-slate-300 sm:px-4">{row.location}</td>
          ...
          <SectionShell>
            <SectionHeading>{MP.portsPeakSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{MP.hillsValleyCavesSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
            ...
            >
              {MP.hillsValleyCavesRows.map((row, i) => (
                <tr key={`${row.name}-${i}`} className="hover:bg-slate-800/40">
                  <RowNumberCell index={i} />
                  <td className="px-3 py-2.5 sm:px-4">{row.name}</td>
                  <td className="px-3 py-2.5 text-slate-300 sm:px-4">{row.location}</td>
          ...
          <SectionShell>
            <SectionHeading>{MP.biosphereSectionTitle}</SectionHeading>
            <p className="mt-2 text-sm font-semibold uppercase text-teal-200/95">{MP.biosphereNote}</p>
          </SectionShell>

          <SectionShell>
            <SectionHeading>{MP.tigerReservesSectionTitle}</SectionHeading>
            {MP.tigerReservesNote ? (
              <p className="mt-2 text-sm font-semibold uppercase text-teal-200/95">{MP.tigerReservesNote}</p>
            ) : null}
            {MP.tigerReservesRows.length > 0 ? (
              <ScrollTable
                header={
                  <tr>
                    <NumberColumnHeader />
              ...
              >
                {MP.tigerReservesRows.map((row, i) => (
                  <tr key={`${row.name}-${i}`} className="hover:bg-slate-800/40">
                    <RowNumberCell index={i} />
                    <td className="px-3 py-2.5 sm:px-4">{row.name}</td>
                    <td className="px-3 py-2.5 text-slate-300 sm:px-4">{row.year}</td>
          ...
          <SectionShell>
            <SectionHeading>{MP.nationalParksSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
            ...
            >
              {MP.nationalParksRows.map((row, i) => (
                <tr key={`${row.name}-${i}`} className="hover:bg-slate-800/40">
                  <RowNumberCell index={i} />
                  <td className="px-3 py-2.5 sm:px-4">{row.name}</td>
                  <td className="px-3 py-2.5 text-slate-300 sm:px-4">{row.year}</td>
          ...
          <SectionShell>
            <SectionHeading>{MP.wildlifeSanctuariesSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
            ...
            >
              {MP.wildlifeSanctuariesRows.map((row, i) => (
                <tr key={`${row.name}-${i}`} className="hover:bg-slate-800/40">
                  <RowNumberCell index={i} />
                  <td className="px-3 py-2.5 sm:px-4">{row.name}</td>
                  <td className="px-3 py-2.5 text-slate-300 sm:px-4">{row.year}</td>
          ...
          <SectionShell>
            <SectionHeading>{MP.zooMuseumsSectionTitle}</SectionHeading>
            {MP.zooMuseumsRows.length > 0 ? (
              <ScrollTable
                header={
                  <tr>
                    <NumberColumnHeader />
              ...
              <p className="mt-2 text-sm font-semibold uppercase text-teal-200/95">{MP.zooMuseumsNote}</p>
            ) : null}
          </SectionShell>

          <SectionShell>
            <SectionHeading>{MP.internationalAirportsSectionTitle}</SectionHeading>
            {MP.internationalAirportsNote ? (
              <p className="mt-2 text-sm font-semibold uppercase text-teal-200/95">{MP.internationalAirportsNote}</p>
            ) : null}
            {MP.internationalAirportsRows.length > 0 ? (
              <ScrollTable
                header={
                  <tr>
                    <NumberColumnHeader />
              ...
              >
                {MP.internationalAirportsRows.map((row, i) => (
                  <tr key={`${row.name}-${i}`} className="hover:bg-slate-800/40">
                    <RowNumberCell index={i} />
                    <td className="px-3 py-2.5 sm:px-4">{row.name}</td>
                    <td className="px-3 py-2.5 text-slate-300 sm:px-4">{row.city}</td>
          ...
          <SectionShell>
            <SectionHeading>{MP.airportsSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{MP.iitSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{MP.universitiesSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{MP.stadiumsSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <ExtendedFactsGrid
              className="mt-0"
              facts={[{ label: 'FAMOUS PERSONS', kind: 'ordered', items: MP.famousPersonsOrdered }]}
            />
          </SectionShell>

          <SectionShell>
            <SectionHeading>{MP.miscellaneousSectionTitle}</SectionHeading>
            <p className="mb-6 text-slate-300 leading-relaxed">{MP.miscellaneousIntroParagraph}</p>
            <div className="space-y-8">
              {MP.miscellaneousBlocks.map((block, blockIndex) => (
