...
import * as DD from '@/assets/general-awareness/states-information/daman-diu'
...
function ExtendedFactsGrid({
  ...
  facts: readonly DD.DamanDiuExtendedFact[]
  className?: string
}) {
  return (
    <dl
...
export default function DamanDiu({ showBackNav = false }: DamanDiuProps) {
  return (
      ...
      <div className="relative mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
        ...
        <div className="space-y-8 sm:space-y-9">
          <SectionShell>
            ...
            <KeyValueGrid items={DD.damanDiuBasicFacts} />
            <ExtendedFactsGrid facts={DD.damanDiuExtendedFacts} />
          </SectionShell>
          ...
          <SectionShell>
            <div className="mb-4 flex items-center gap-3">
              ...
              <SectionHeading>{DD.templesSectionTitle}</SectionHeading>
            </div>
            <ScrollTable
              header={
                <tr>
          ...
          <SectionShell>
            <SectionHeading>{DD.industriesSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{DD.powerPlantsSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <ExtendedFactsGrid className="mt-0" facts={[{ label: 'RIVERS', kind: 'ordered', items: DD.riversOrdered }]} />
          </SectionShell>

          <SectionShell>
            <SectionHeading>{DD.lakesSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{DD.projectsDamsSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{DD.waterfallsBeachSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
            ...
            >
              {DD.waterfallsBeachRows.map((row, i) => (
                <tr key={`${row.name}-${i}`} className="hover:bg-slate-800/40">
                  <RowNumberCell index={i} />
                  <td className="px-3 py-2.5 sm:px-4">{row.name}</td>
                  <td className="px-3 py-2.5 text-slate-300 sm:px-4">{row.location}</td>
          ...
          <SectionShell>
            <SectionHeading>{DD.portsPeakSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{DD.hillsValleyCavesSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
            ...
            >
              {DD.hillsValleyCavesRows.map((row, i) => (
                <tr key={`${row.name}-${i}`} className="hover:bg-slate-800/40">
                  <RowNumberCell index={i} />
                  <td className="px-3 py-2.5 sm:px-4">{row.name}</td>
                  <td className="px-3 py-2.5 text-slate-300 sm:px-4">{row.location}</td>
          ...
          <SectionShell>
            <SectionHeading>{DD.biosphereSectionTitle}</SectionHeading>
            <p className="mt-2 text-sm font-semibold uppercase text-teal-200/95">{DD.biosphereNote}</p>
          </SectionShell>

          <SectionShell>
            <SectionHeading>{DD.tigerReservesSectionTitle}</SectionHeading>
            {DD.tigerReservesNote ? (
              <p className="mt-2 text-sm font-semibold uppercase text-teal-200/95">{DD.tigerReservesNote}</p>
            ) : null}
            {DD.tigerReservesRows.length > 0 ? (
              <ScrollTable
                header={
                  <tr>
                    <NumberColumnHeader />
              ...
              >
                {DD.tigerReservesRows.map((row, i) => (
                  <tr key={`${row.name}-${i}`} className="hover:bg-slate-800/40">
                    <RowNumberCell index={i} />
                    <td className="px-3 py-2.5 sm:px-4">{row.name}</td>
                    <td className="px-3 py-2.5 text-slate-300 sm:px-4">{row.year}</td>
          ...
          <SectionShell>
            <SectionHeading>{DD.nationalParksSectionTitle}</SectionHeading>
            {DD.nationalParksNote ? (
              <p className="mt-2 text-sm font-semibold uppercase text-teal-200/95">{DD.nationalParksNote}</p>
            ) : null}
            {DD.nationalParksRows.length > 0 ? (
              <ScrollTable
                header={
                  <tr>
                    <NumberColumnHeader />
              ...
              >
                {DD.nationalParksRows.map((row, i) => (
                  <tr key={`${row.name}-${i}`} className="hover:bg-slate-800/40">
                    <RowNumberCell index={i} />
                    <td className="px-3 py-2.5 sm:px-4">{row.name}</td>
                    <td className="px-3 py-2.5 text-slate-300 sm:px-4">{row.year}</td>
          ...
          <SectionShell>
            <SectionHeading>{DD.wildlifeSanctuariesSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
            ...
            >
              {DD.wildlifeSanctuariesRows.map((row, i) => (
                <tr key={`${row.name}-${i}`} className="hover:bg-slate-800/40">
                  <RowNumberCell index={i} />
                  <td className="px-3 py-2.5 sm:px-4">{row.name}</td>
                  <td className="px-3 py-2.5 text-slate-300 sm:px-4">{row.year}</td>
          ...
          <SectionShell>
            <SectionHeading>{DD.zooMuseumsSectionTitle}</SectionHeading>
            {DD.zooMuseumsNote ? (
              <p className="mt-2 text-sm font-semibold uppercase text-teal-200/95">{DD.zooMuseumsNote}</p>
            ) : null}
            {DD.zooMuseumsRows.length > 0 ? (
              <ScrollTable
                header={
                  <tr>
                    <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{DD.internationalParksSectionTitle}</SectionHeading>
            <p className="mt-2 text-sm font-semibold uppercase text-teal-200/95">{DD.internationalParksNote}</p>
          </SectionShell>

          <SectionShell>
            <SectionHeading>{DD.airportsSectionTitle}</SectionHeading>
            {DD.airportsRows.length > 0 ? (
              <ScrollTable
                header={
                  <tr>
                    <NumberColumnHeader />
              ...
              <p className="mt-2 text-sm font-semibold uppercase text-teal-200/95">{DD.airportsNote}</p>
            ) : null}
          </SectionShell>

          <SectionShell>
            <SectionHeading>{DD.stadiumsSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{DD.iitSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{DD.universitiesSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <ExtendedFactsGrid
              className="mt-0"
              facts={[{ label: 'FAMOUS PERSONS', kind: 'ordered', items: DD.famousPersonsOrdered }]}
            />
          </SectionShell>

          <SectionShell>
            <SectionHeading>{DD.miscellaneousSectionTitle}</SectionHeading>
            <p className="mb-6 whitespace-pre-line text-slate-300 leading-relaxed">{DD.miscellaneousIntroParagraph}</p>
            <div className="space-y-8">
              {DD.miscellaneousBlocks.map((block, blockIndex) => (
