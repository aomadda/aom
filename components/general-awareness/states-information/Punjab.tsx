...
import * as PU from '@/assets/general-awareness/states-information/punjab'
...
function ExtendedFactsGrid({
  ...
  facts: readonly PU.PunjabExtendedFact[]
  className?: string
}) {
  return (
    <dl
...
const Punjab = ({ showBackNav = false }: PunjabProps) => {
  return (
      ...
      <div className="relative mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
        ...
        <div className="space-y-8 sm:space-y-9">
          <SectionShell>
            ...
            <KeyValueGrid items={PU.punjabBasicFacts} />
            <ExtendedFactsGrid facts={PU.punjabExtendedFacts} />
          </SectionShell>
          ...
          <SectionShell>
            ...
            {PU.internationalAirportsRows.length > 0 ? (
              ...
              >
                {PU.internationalAirportsRows.map((row, i) => (
                  <tr key={`${row.name}-${i}`} className="hover:bg-slate-800/40">
                    <RowNumberCell index={i} />
                    <td className="px-3 py-2.5 sm:px-4">{row.name}</td>
                    <td className="px-3 py-2.5 text-slate-300 sm:px-4">{row.city}</td>
          ...
          <SectionShell>
            ...
            <p className="mb-6 text-slate-300 leading-relaxed">{PU.miscellaneousIntroParagraph}</p>
