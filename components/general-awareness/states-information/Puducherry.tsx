...
import * as PU from '@/assets/general-awareness/states-information/puducherry'

type PuducherryProps = {
  showBackNav?: boolean
}
...
function ExtendedFactsGrid({
  ...
  facts: readonly PU.PuducherryExtendedFact[]
  className?: string
}) {
  return (
    <dl
...
export default function Puducherry({ showBackNav = false }: PuducherryProps) {
  return (
      ...
      <div className="relative mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
        ...
        <div className="space-y-8 sm:space-y-9">
          <SectionShell>
            ...
            <KeyValueGrid items={PU.puducherryBasicFacts} />
            <ExtendedFactsGrid facts={PU.puducherryExtendedFacts} />
          </SectionShell>
          ...
          <SectionShell>
            <div className="mb-4 flex items-center gap-3">
              ...
              <SectionHeading>{PU.templesSectionTitle}</SectionHeading>
            </div>
            <ScrollTable
              header={
                <tr>
          ...
          <SectionShell>
            <SectionHeading>{PU.industriesSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{PU.powerPlantsSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <ExtendedFactsGrid className="mt-0" facts={[{ label: 'RIVERS', kind: 'ordered', items: PU.riversOrdered }]} />
          </SectionShell>

          <SectionShell>
            <SectionHeading>{PU.lakesSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{PU.projectsDamsSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{PU.waterfallsBeachSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
            ...
            >
              {PU.waterfallsBeachRows.map((row, i) => (
                <tr key={`${row.name}-${i}`} className="hover:bg-slate-800/40">
                  <RowNumberCell index={i} />
                  <td className="px-3 py-2.5 sm:px-4">{row.name}</td>
                  <td className="px-3 py-2.5 text-slate-300 sm:px-4">{row.location}</td>
          ...
          <SectionShell>
            <SectionHeading>{PU.portsPeakSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{PU.hillsValleyCavesSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
            ...
            >
              {PU.hillsValleyCavesRows.map((row, i) => (
                <tr key={`${row.name}-${i}`} className="hover:bg-slate-800/40">
                  <RowNumberCell index={i} />
                  <td className="px-3 py-2.5 sm:px-4">{row.name}</td>
                  <td className="px-3 py-2.5 text-slate-300 sm:px-4">{row.location}</td>
          ...
          <SectionShell>
            <SectionHeading>{PU.biosphereSectionTitle}</SectionHeading>
            <p className="mt-2 text-sm font-semibold uppercase text-teal-200/95">{PU.biosphereNote}</p>
          </SectionShell>

          <SectionShell>
            <SectionHeading>{PU.tigerReservesSectionTitle}</SectionHeading>
            {PU.tigerReservesNote ? (
              <p className="mt-2 text-sm font-semibold uppercase text-teal-200/95">{PU.tigerReservesNote}</p>
            ) : null}
            {PU.tigerReservesRows.length > 0 ? (
              <ScrollTable
                header={
                  <tr>
                    <NumberColumnHeader />
              ...
              >
                {PU.tigerReservesRows.map((row, i) => (
                  <tr key={`${row.name}-${i}`} className="hover:bg-slate-800/40">
                    <RowNumberCell index={i} />
                    <td className="px-3 py-2.5 sm:px-4">{row.name}</td>
                    <td className="px-3 py-2.5 text-slate-300 sm:px-4">{row.year}</td>
          ...
          <SectionShell>
            <SectionHeading>{PU.nationalParksSectionTitle}</SectionHeading>
            {PU.nationalParksNote ? (
              <p className="mt-2 text-sm font-semibold uppercase text-teal-200/95">{PU.nationalParksNote}</p>
            ) : null}
            {PU.nationalParksRows.length > 0 ? (
              <ScrollTable
                header={
                  <tr>
                    <NumberColumnHeader />
              ...
              >
                {PU.nationalParksRows.map((row, i) => (
                  <tr key={`${row.name}-${i}`} className="hover:bg-slate-800/40">
                    <RowNumberCell index={i} />
                    <td className="px-3 py-2.5 sm:px-4">{row.name}</td>
                    <td className="px-3 py-2.5 text-slate-300 sm:px-4">{row.year}</td>
          ...
          <SectionShell>
            <SectionHeading>{PU.wildlifeSanctuariesSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
            ...
            >
              {PU.wildlifeSanctuariesRows.map((row, i) => (
                <tr key={`${row.name}-${i}`} className="hover:bg-slate-800/40">
                  <RowNumberCell index={i} />
                  <td className="px-3 py-2.5 sm:px-4">{row.name}</td>
                  <td className="px-3 py-2.5 text-slate-300 sm:px-4">{row.year}</td>
          ...
          <SectionShell>
            <SectionHeading>{PU.zooMuseumsSectionTitle}</SectionHeading>
            {PU.zooMuseumsNote ? (
              <p className="mt-2 text-sm font-semibold uppercase text-teal-200/95">{PU.zooMuseumsNote}</p>
            ) : null}
            {PU.zooMuseumsRows.length > 0 ? (
              <ScrollTable
                header={
                  <tr>
                    <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{PU.internationalAirportsSectionTitle}</SectionHeading>
            <p className="mt-2 text-sm font-semibold uppercase text-teal-200/95">{PU.internationalAirportsNote}</p>
          </SectionShell>

          <SectionShell>
            <SectionHeading>{PU.airportsSectionTitle}</SectionHeading>
            {PU.airportsRows.length > 0 ? (
              <ScrollTable
                header={
                  <tr>
                    <NumberColumnHeader />
              ...
              <p className="mt-2 text-sm font-semibold uppercase text-teal-200/95">{PU.airportsNote}</p>
            ) : null}
          </SectionShell>

          <SectionShell>
            <SectionHeading>{PU.stadiumsSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{PU.iitSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{PU.universitiesSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <ExtendedFactsGrid
              className="mt-0"
              facts={[{ label: 'FAMOUS PERSONS', kind: 'ordered', items: PU.famousPersonsOrdered }]}
            />
          </SectionShell>

          <SectionShell>
            <SectionHeading>{PU.miscellaneousSectionTitle}</SectionHeading>
            <p className="mb-6 whitespace-pre-line text-slate-300 leading-relaxed">{PU.miscellaneousIntroParagraph}</p>
            <div className="space-y-8">
              {PU.miscellaneousBlocks.map((block, blockIndex) => (
