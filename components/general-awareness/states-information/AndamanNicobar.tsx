...
import * as AN from '@/assets/general-awareness/states-information/andaman-nicobar'

type AndamanNicobarProps = {
  showBackNav?: boolean
}
...
function ExtendedFactsGrid({
  ...
  facts: readonly AN.AndamanNicobarExtendedFact[]
  className?: string
}) {
  return (
    <dl
...
const AndamanNicobar = ({ showBackNav = false }: AndamanNicobarProps) => {
  return (
      ...
      <div className="relative mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
        ...
        <div className="space-y-8 sm:space-y-9">
          <SectionShell>
            ...
            <KeyValueGrid items={AN.andamanNicobarBasicFacts} />
            <ExtendedFactsGrid facts={AN.andamanNicobarExtendedFacts} />
          </SectionShell>
          ...
          <SectionShell>
            <div className="mb-4 flex items-center gap-3">
              ...
              <SectionHeading>{AN.templesSectionTitle}</SectionHeading>
            </div>
            <ScrollTable
              header={
                <tr>
          ...
          <SectionShell>
            <SectionHeading>{AN.industriesSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{AN.powerPlantsSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <ExtendedFactsGrid className="mt-0" facts={[{ label: 'RIVERS', kind: 'ordered', items: AN.riversOrdered }]} />
          </SectionShell>

          <SectionShell>
            <SectionHeading>{AN.lakesSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{AN.projectsDamsSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{AN.waterfallsBeachSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
            ...
            >
              {AN.waterfallsBeachRows.map((row, i) => (
                <tr key={`${row.name}-${i}`} className="hover:bg-slate-800/40">
                  <RowNumberCell index={i} />
                  <td className="px-3 py-2.5 sm:px-4">{row.name}</td>
                  <td className="px-3 py-2.5 text-slate-300 sm:px-4">{row.location}</td>
          ...
          <SectionShell>
            <SectionHeading>{AN.portsPeakSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{AN.hillsValleyCavesSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
            ...
            >
              {AN.hillsValleyCavesRows.map((row, i) => (
                <tr key={`${row.name}-${i}`} className="hover:bg-slate-800/40">
                  <RowNumberCell index={i} />
                  <td className="px-3 py-2.5 sm:px-4">{row.name}</td>
                  <td className="px-3 py-2.5 text-slate-300 sm:px-4">{row.location}</td>
          ...
          <SectionShell>
            <SectionHeading>{AN.biosphereSectionTitle}</SectionHeading>
            <p className="mt-2 text-sm font-semibold uppercase text-teal-200/95">{AN.biosphereNote}</p>
          </SectionShell>

          <SectionShell>
            <SectionHeading>{AN.tigerReservesSectionTitle}</SectionHeading>
            {AN.tigerReservesNote ? (
              <p className="mt-2 text-sm font-semibold uppercase text-teal-200/95">{AN.tigerReservesNote}</p>
            ) : null}
            {AN.tigerReservesRows.length > 0 ? (
              <ScrollTable
                header={
                  <tr>
                    <NumberColumnHeader />
              ...
              >
                {AN.tigerReservesRows.map((row, i) => (
                  <tr key={`${row.name}-${i}`} className="hover:bg-slate-800/40">
                    <RowNumberCell index={i} />
                    <td className="px-3 py-2.5 sm:px-4">{row.name}</td>
                    <td className="px-3 py-2.5 text-slate-300 sm:px-4">{row.year}</td>
          ...
          <SectionShell>
            <SectionHeading>{AN.nationalParksSectionTitle}</SectionHeading>
            {AN.nationalParksNote ? (
              <p className="mt-2 text-sm font-semibold uppercase text-teal-200/95">{AN.nationalParksNote}</p>
            ) : null}
            {AN.nationalParksRows.length > 0 ? (
              <ScrollTable
                header={
                  <tr>
                    <NumberColumnHeader />
              ...
              >
                {AN.nationalParksRows.map((row, i) => (
                  <tr key={`${row.name}-${i}`} className="hover:bg-slate-800/40">
                    <RowNumberCell index={i} />
                    <td className="px-3 py-2.5 sm:px-4">{row.name}</td>
                    <td className="px-3 py-2.5 text-slate-300 sm:px-4">{row.year}</td>
          ...
          <SectionShell>
            <SectionHeading>{AN.wildlifeSanctuariesSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
            ...
            >
              {AN.wildlifeSanctuariesRows.map((row, i) => (
                <tr key={`${row.name}-${i}`} className="hover:bg-slate-800/40">
                  <RowNumberCell index={i} />
                  <td className="px-3 py-2.5 sm:px-4">{row.name}</td>
                  <td className="px-3 py-2.5 text-slate-300 sm:px-4">{row.year}</td>
          ...
          <SectionShell>
            <SectionHeading>{AN.zooMuseumsSectionTitle}</SectionHeading>
            {AN.zooMuseumsNote ? (
              <p className="mt-2 text-sm font-semibold uppercase text-teal-200/95">{AN.zooMuseumsNote}</p>
            ) : null}
            {AN.zooMuseumsRows.length > 0 ? (
              <ScrollTable
                header={
                  <tr>
                    <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{AN.internationalAirportsSectionTitle}</SectionHeading>
            {AN.internationalAirportsNote ? (
              <p className="mt-2 text-sm font-semibold uppercase text-teal-200/95">{AN.internationalAirportsNote}</p>
            ) : null}
            {AN.internationalAirportsRows.length > 0 ? (
              <ScrollTable
                header={
                  <tr>
                    <NumberColumnHeader />
              ...
              >
                {AN.internationalAirportsRows.map((row, i) => (
                  <tr key={`${row.name}-${i}`} className="hover:bg-slate-800/40">
                    <RowNumberCell index={i} />
                    <td className="px-3 py-2.5 sm:px-4">{row.name}</td>
                    <td className="px-3 py-2.5 text-slate-300 sm:px-4">{row.city}</td>
          ...
          <SectionShell>
            <SectionHeading>{AN.airportsSectionTitle}</SectionHeading>
            {AN.airportsRows.length > 0 ? (
              <ScrollTable
                header={
                  <tr>
                    <NumberColumnHeader />
              ...
              <p className="mt-2 text-sm font-semibold uppercase text-teal-200/95">{AN.airportsNote}</p>
            ) : null}
          </SectionShell>

          <SectionShell>
            <SectionHeading>{AN.iitSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{AN.universitiesSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <SectionHeading>{AN.stadiumsSectionTitle}</SectionHeading>
            <ScrollTable
              header={
                <tr>
                  <NumberColumnHeader />
          ...
          <SectionShell>
            <ExtendedFactsGrid
              className="mt-0"
              facts={[{ label: 'FAMOUS PERSONS', kind: 'ordered', items: AN.famousPersonsOrdered }]}
            />
          </SectionShell>

          <SectionShell>
            <SectionHeading>{AN.miscellaneousSectionTitle}</SectionHeading>
            <p className="mb-6 text-slate-300 leading-relaxed">{AN.miscellaneousIntroParagraph}</p>
            <div className="space-y-8">
              {AN.miscellaneousBlocks.map((block, blockIndex) => (
