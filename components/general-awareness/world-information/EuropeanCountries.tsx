...
import * as EC from '@/assets/general-awareness/world-information/european-countries'

type EuropeanCountriesProps = {
  showBackNav?: boolean
}

const detailFields: readonly {
  label: string
  key: keyof EC.EuropeanCountryRow
...
function CountryCard({ index, row }: { index: number; row: EC.EuropeanCountryRow }) {
  return (
...
const EuropeanCountries = ({ showBackNav = false }: EuropeanCountriesProps) => {
  return (
      ...
      <div className="relative mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
        <header className="mb-10 text-center sm:mb-12">
          ...
          <h1 className="mt-3 bg-linear-to-r from-emerald-100 via-teal-100 to-emerald-200 bg-clip-text text-xl font-bold uppercase tracking-tight text-transparent sm:text-2xl md:text-3xl">
            {EC.europeanCountriesHeaderLine}
          </h1>
          <div
        ...
        <div className="space-y-6 sm:space-y-7">
          {EC.europeanCountriesRows.map((row, i) => (
