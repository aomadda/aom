...
import * as AC from '@/assets/general-awareness/world-information/australian-countries'

type AustralianCountriesProps = {
  showBackNav?: boolean
}

const detailFields: readonly {
  label: string
  key: keyof AC.AustralianCountryRow
  Icon: React.ComponentType<{ className?: string; strokeWidth?: number }>
}[] = [
  { label: 'Capital', key: 'capital', Icon: Building2 },
  { label: 'Currency', key: 'currency', Icon: Coins },
  { label: 'Head of State', key: 'headOfState', Icon: Crown },
  { label: 'Head of Government', key: 'headOfGovernment', Icon: Landmark },
]

function CountryCard({ index, row }: { index: number; row: AC.AustralianCountryRow }) {
  return (
...
const AustralianCountries = ({ showBackNav = false }: AustralianCountriesProps) => {
  return (
      ...
      <div className="relative mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
        <header className="mb-10 text-center sm:mb-12">
          ...
          <h1 className="mt-3 bg-linear-to-r from-emerald-100 via-teal-100 to-emerald-200 bg-clip-text text-xl font-bold uppercase tracking-tight text-transparent sm:text-2xl md:text-3xl">
            {AC.australianCountriesHeaderLine}
          </h1>
          <div
        ...
        <div className="space-y-6 sm:space-y-7">
          {AC.australianCountriesRows.map((row, i) => (
...
export default AustralianCountries
