...
import { nationalParksByState } from '@/assets/general-awareness/national-parks'

type NationalParksProps = {
  showBackNav?: boolean
}

const totalParkCount = nationalParksByState.reduce(
  (n, g) => n + g.parks.length,
  0,
)
...
function filterGroups(query: string) {
  ...
  return nationalParksByState
    .map((group) => ({
      ...group,
      parks: group.parks.filter((row) => {
        if (group.state.toLowerCase().includes(q)) return true
        if (row.parkName.toLowerCase().includes(q)) return true
        if (row.yearEstablished.includes(raw)) return true
        return false
      }),
    }))
    ...
    >
      ...
      <div className="relative flex items-center gap-2 sm:gap-3">
        ...
        <input
          ...
          placeholder="National Parks of India"
...
const NationalParks = ({ showBackNav = false }: NationalParksProps) => {
  ...
  const visibleCount = useMemo(
    () => filteredGroups.reduce((n, g) => n + g.parks.length, 0),
    [filteredGroups],
  )
    ...
    <div className="min-h-screen bg-linear-to-br from-slate-900 via-slate-800 to-emerald-950/90 py-10 px-3 sm:px-6 lg:px-8">
        ...
        >
          <div className="rounded-2xl border border-slate-600/50 bg-slate-900/92 p-1.5 shadow-[0_12px_40px_-8px_rgba(0,0,0,0.65)] backdrop-blur-xl sm:p-2">
            <SearchBarField
              value={search}
              onChange={setSearch}
              inputId="national-parks-search-floating"
              variant="floating"
            />
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div
          ...
          role="search"
          aria-label="Filter national parks by state, name, or year"
        >
          <SearchBarField
            value={search}
            onChange={setSearch}
            inputId="national-parks-search"
          />
          {isFiltering ? (
                  ...
                  <table className="w-full min-w-[640px] text-left border-collapse">
                    <thead>
                      <tr className="border-b border-slate-600/60 bg-slate-900/40">
                        ...
                        <th
                          scope="col"
                          className="px-3 py-3 sm:px-5 text-xs font-semibold uppercase tracking-wide text-emerald-400/90"
                        >
                          National park name
                        </th>
                        <th
